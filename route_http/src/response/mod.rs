...
// pub trait ResponseExtTrait<T>
// where
//   T: Serialize,
// {
//   fn from_req(req: &HttpRequest, body: T) -> Self;
// }
//
// impl<T> ResponseExtTrait<T> for Response<String>
// where
//   T: Serialize,
// {
//   fn from_req(req: &HttpRequest, _body: T) -> Self {
//     let accept = req.headers().get("Accept");
//     let content_type = req.headers().get("Content-Type");
//
//     let header_used = accept.or(content_type);
//
//     match header_used {
//       None => unimplemented!(),
//       Some(header) => {
//         let header = header.to_str().unwrap();
//         let my_mime = mime::Mime::from_str(header).unwrap();
//         let mut res = Response::builder();
//         let res_headers = res.headers_mut().unwrap();
//         res_headers.insert("Content-Type", header.parse().unwrap());
//
//         let body = if my_mime != mime::APPLICATION_JSON {
//           serde_json::to_string(&_body).unwrap()
//         } else {
//           res_headers.insert("Content-Type", "plain/text".parse().unwrap());
//           "Unsupported Media Type".to_string()
//         };
//         res_headers.insert("Content-Length", body.bytes().len().into());
