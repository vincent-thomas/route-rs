...
pub use lambda_http::{lambda_runtime::Diagnostic, IntoResponse, Request};
use std::{error::Error, fmt::Debug};
use titan_core::Service;

pub async fn run<'a, S>(handler: S) -> Result<(), Box<dyn Error + Send + Sync>>
where
  S: Service<Request>,
  S::Future: Send + 'a,
  S::Response: IntoResponse,
  S::Error: Debug + Into<Diagnostic>,
{
